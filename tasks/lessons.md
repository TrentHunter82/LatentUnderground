...
### [Claude-4] Mock the COMPLETE hook context shape via a shared factory, not a partial inline object
- What happened: 8 test files mocked `useTheme` with only `{ theme, mode, toggleTheme, setTheme }`, omitting the orthogonal skin axis (`skin/setSkin/cycleSkin/skins`). Tests stayed green because no rendered component in those files consumed the skin keys — but it was a latent trap: the moment one of them rendered `SettingsPanel`→`ThemePicker`, the picker would silently be non-functional (undefined skin, no-op setSkin) and never fail a test.
- Root cause: Each test hand-rolled its own `useTheme` mock, so the mock and the real context contract drifted independently. The full registry was also hardcoded in two files (drift risk if SKINS changes).
- Rule: For any context/hook with a non-trivial shape, add ONE `createXMock()` factory in test-utils.jsx that returns the full shape (importing real constants like `SKINS` so it can't drift) and route every `vi.mock` through it. Mirrors the existing `createApiMock`/`createProjectQueryMock` pattern. A partial mock that "happens to pass" is a regression waiting for the next render-tree change.

### [Claude-4] A roving-tabindex widget inside a focus trap breaks the trap's querySelectorAll
- What happened: SettingsPanel's focus trap collected focusables with `querySelectorAll('button, ...')`. ThemePicker (WAI-ARIA radiogroup) renders inactive radios as `<button tabindex="-1">`. The bare `button` selector matched them, so Tab could land on a radio that is deliberately out of the tab order.
- Root cause: The selector only excluded `tabindex="-1"` on the generic `[tabindex]` clause, not on the element-type clauses (`button`, `input`, …). Elements removed from the tab order via `tabindex="-1"` were still matched.
- Rule: Focus-trap/focusable selectors must exclude `:not([tabindex="-1"])` on EVERY element-type clause (`button:not([tabindex="-1"]), input:not([tabindex="-1"]), …`), not just the catch-all `[tabindex]`. Any roving-tabindex widget (radiogroup, toolbar, menu) placed inside a trap will otherwise leak its inactive items into the tab sequence.

### [Claude-4] Per-skin CSS must override interactive pseudo-states and decorative classes, not just base rules
- What happened: Obsidian/Blueprint skins re-themed `.btn-neon-danger` but not `.btn-neon-danger:hover`, and Blueprint omitted `.retro-panel-glow`. Those base rules hardcode the Analog mint palette, so hovering a danger button (or any glow panel) in another skin flashed mint green — a live, user-visible palette clash.
- Root cause: Skin override blocks were written by eyeballing the base component classes and missed the `:hover`/`:focus` pseudo-state variants and a decorative class that also hardcode color.
- Rule: When reskinning, audit the base stylesheet for EVERY variant of each overridden selector — `:hover`, `:focus`, `::placeholder`, and sibling decorative classes (`*-glow`) — and override them per skin too. A skin that overrides `.x` but not `.x:hover` silently inherits the default palette on interaction.

### [Claude-4] Enforce single-source-of-truth registries that can't be imported with an invariant test
- What happened: The skin id list lived in `lib/themes.js` (SKIN_IDS) but had to be duplicated in `index.html` (pre-hydration bootstrap, can't import modules) and `index.css` (per-skin blocks). A new skin added only to themes.js would be silently rejected by the bootstrap and render as the base skin — no error, just a flash.
- Root cause: Some consumers legitimately cannot import the registry (inline HTML script, static CSS). The "keep in sync" comment relied on human memory with no mechanical check.
- Rule: When a constant must be duplicated outside the module system, add a vitest guard that reads the other files (`fs.readFileSync`) and asserts they match the canonical export. The drift then fails CI loudly instead of shipping a silent visual bug. See `theme-registry-sync.test.js`.
...
### [Claude-2] Re-snapshot shared files before a big rewrite — concurrent agents mutate them
- What happened: I read index.css (1058 lines), then composed a full token-driven rewrite from that read. Between my read and my work, a concurrent agent (Claude-4) grew the file to 1072 lines (added per-skin `.btn-neon-danger:hover` rules + a blueprint-dark `.retro-panel-glow`). My rewrite silently DROPPED those additions — a real regression I couldn't see (tests don't assert computed CSS).
- Root cause: In a shared working tree with live agents, an early read goes stale fast. A full-file Write composed from a stale read clobbers others' in-flight edits.
- Rule: Before a large rewrite of a shared file, take a FRESH snapshot immediately prior (cp to a baseline), and diff/verify against THAT snapshot, not your earlier read. Lock the file (AGENT_STATUS + bus) and re-read right before writing.

### [Claude-2] Prove a CSS refactor is value-exact with a deterministic resolver, not by eye
- What happened: Refactored every component class to read CSS custom properties (tokens) and collapsed per-skin overrides into token blocks. Risk: an invisible color transcription error across ~50 tokens × 6 skin×mode contexts. jsdom can't compute var() cascades, and no test checks rendered colors.
- Root cause: Token refactors move values around; a single wrong hex is a silent visual regression.
- Rule: Write a small Node resolver (.claude/verify-css-equiv.cjs) that, per skin×mode, resolves the effective value of every component property in BOTH old and new CSS and diffs them. Handle the real cascade: fold `border-color` longhand into the `border` shorthand, and seed `:hover/:focus` from the resting rule (pseudo-states inherit border). Whitelist deliberate changes. Result: "0 unexpected mismatches / 858 checks" is a far stronger guarantee than a visual spot-check.

### [Claude-2] For a uniformly-heavy test file, set ONE file-level timeout, not per-test trailing args
- What happened: `phase16-accessibility-audit.test.jsx` failed in the full suite (FileEditor axe audit timed out at 5000ms) but passed in isolation — the classic resource-pressure flake (lessons #199/#411). Every `it()` in that file does `await import('../components/X')` + a full `axe()` audit, so any of them can lose the race under load; FileEditor just lost it this run.
- Root cause: The whole file is uniformly heavy, yet relied on the 5s default timeout. Patching the one test that happened to fail (`}, 15000)`) would leave the other ~16 equally-flaky tests unguarded — whack-a-mole.
- Rule: When an entire test file is uniformly heavy (dynamic import + axe, or similar), add a single `vi.setConfig({ testTimeout: 15000 })` at the top instead of per-test trailing timeouts. It covers every test AND any future additions, and is far less noisy. Reserve the per-test `}, 15000)` (lesson #199) for files where only one or two tests are heavy.
- Update (Claude-4): SECOND confirmed instance — `phase15-features.test.jsx` (35 tests, every one a `render()` + async `act()`, FileEditor dynamically pulls react-markdown) flaked on the FileEditor-404 tests at the 5s default under full-suite load (35/35 green in isolation). Applied the same file-level `vi.setConfig({ testTimeout: 15000 })`. This is now the standard fix for any uniformly-heavy dynamic-import+render file; suspect it FIRST when a whole-suite run fails 1-2 tests at exactly 5000ms that pass alone.

### [Claude-2] A full reskin is NOT cheap even with tokens — budget for it
- What happened: The token refactor tightened base CSS ~2KB, but the Terminal reskin (a totally different palette) overrode ~50 tokens per variant (dark+light), adding ~5KB and pushing built CSS to 72.5KB — over the 72KB budget (two separate budget asserts: bundle-size.test.js AND phase27-performance.test.jsx).
- Root cause: "Tokens make a 3rd skin cheap" only holds when a skin overrides a FEW tokens. A distinct palette overrides most of them, so its cost ≈ its token count, not zero.
- Rule: When adding a full reskin, expect ~4-6KB and update EVERY CSS-budget assertion (grep for the old `* 1024` budget across tests, not just one file). Bump with a justification comment; don't silently raise it.

### [Claude-3] `vi.setConfig({ testTimeout })` is UNRELIABLE under parallel load — set testTimeout in vite.config.js (SUPERSEDES the per-file/per-test timeout lessons above)
- What happened: A full-suite run failed 4 heavy dynamic-import tests (phase15/phase5/phase17) while all 3 files passed cleanly in isolation. Critically, `phase15-features.test.jsx` already had a file-level `vi.setConfig({ testTimeout: 15000 })` (the "standard fix" from the lesson above) yet its FileEditor tests STILL died at the **5000ms default** — proving the file-level setConfig did not take effect. phase17's per-test `}, 15000)` was itself insufficient (a <2s isolated test stalled >15s under load). The suite had accreted 24 per-test `}, NNNNN)` caps + 2 `vi.setConfig`, all inconsistent (5000/10000/15000/20000).
- Root cause: `vi.setConfig` mutates the worker's runtime config; under full-suite parallel execution it is not reliably applied before a racing test runs, so the test falls back to the static 5000ms default. Per-test trailing timeouts are whack-a-mole — they only guard the one test that flaked last time, and any value below the real load-tail just re-flakes.
- Rule: Put ONE authoritative `testTimeout` (+`hookTimeout`) in the `test` block of `vite.config.js` — read once at startup, applied deterministically to every worker, cannot be raced. Then DELETE every per-test `}, NNNNN)` cap and `vi.setConfig({testTimeout})` call: below the global they are latent flakes, equal to it they are noise. Genuine infinite hangs (full-App renders) are already `describe.skip`'d (lesson #411), so a generous global ceiling (30000) only ever fires on a real hang. Removing the trailing timeout does NOT weaken perf benchmarks — those assert via `expect(elapsed).toBeLessThan(N)` in the body, independent of the test timeout. Verified: 806 pass / 24 skip / 0 fail across two consecutive full runs after the change. NOTE: this reverses the two "use `vi.setConfig`/per-test timeout" lessons above — do NOT re-add them.

### [Claude-3] An e2e "persistence" test that writes the WRONG storage key is a green tautology testing nothing
- What happened: `e2e/app.spec.js` "persists theme preference across page loads" did `localStorage.setItem('theme','light')`, reloaded, then asserted `localStorage.getItem('theme') === 'light'`. It passed forever — but the app reads `latent-theme` (useTheme.jsx `STORAGE_KEY`), NOT `theme`. So the test set a key the app ignores and only asserted that the key it just set still existed. It never verified the app honoured the preference; it would stay green even if theme persistence were completely broken.
- Root cause: The assertion checked the test's own side effect (the localStorage write) instead of the app's observable behaviour (the `.light` class appearing on `<html>`). Wrong key + self-referential assertion = a test that can never fail for the right reason.
- Rule: A persistence/restore test must assert the APP's reaction to the stored value, not that the value persists. Seed the EXACT key the code reads (grep the source for the storage key — don't guess `theme` vs `latent-theme`), reload, then assert the rendered/applied effect (`await expect(page.locator('html')).toHaveClass(/light/)`). Same principle for skins: assert `data-skin`, not just that `latent-skin` round-trips. jsdom can't run index.html's pre-paint bootstrap <script>, so this class of behaviour MUST be covered in Playwright (see `e2e/theme-skins.spec.js`).
- Aside (env): the bundled Chromium download was blocked in this sandbox; ran the specs against system Edge via a throwaway `channel:'msedge'` config (deleted after). The specs are browser-agnostic and run on the committed config's chromium project in CI.

### [Claude-2] Attribute-only e2e is blind to a deleted/broken skin CSS block — assert the painted cascade
- What happened: The theme e2e had 13 tests but every one asserted *wiring* (`data-skin` attribute, `.light` class, radio `aria-checked`). None read a rendered color. A skin whose entire token block in index.css was deleted (or had a broken `var(--x)`) would still set `data-skin="terminal"` and pass every test while rendering identically to analog — a silent, user-visible regression no test at any level (jsdom unit/integration, backend validator, or these e2e) could catch.
- Root cause: jsdom can't compute `var()` cascades (lesson #446), so the jsdom suites can't check colors; the e2e then defaulted to the same attribute assertions jsdom already does, leaving the actual visual contract unverified everywhere. "13 e2e tests" looked thorough but had a hole exactly where the only browser-capable layer should have closed it.
- Rule: For a token-driven theme system, add ONE real-browser test that reads `getComputedStyle` for the resolved tokens AND a painted property (`getComputedStyle(document.body).backgroundColor`) per skin, and asserts the set of painted values is distinct across all skins (+ light≠dark per skin). Distinctness across N skins is non-vacuous: a missing block collapses that skin onto the base palette, shrinking the distinct-set below N and failing the assertion. Before calling a feature "verified at all levels," ask which layer is the ONLY one that can check the actual contract (here: painted color in a browser) and confirm a test there exercises it. (Complements Claude-4's `theme-contrast-audit.test.js`, which resolves the cascade in Node for WCAG ratios; this asserts it in a real engine.)

### [Claude-4] Audit WCAG contrast in a node-side vitest by resolving the var() cascade yourself — and prove the guard can fail
- What happened: The Hardening phase needed an automated WCAG AA contrast audit for every skin × mode. jsdom does NOT compute `var()` cascades or `getComputedStyle` colors (lessons #445/#446), so a normal jsdom test can't read the effective body/accent colors at all.
- Root cause: Theme colors live in layered custom properties (`:root` → `html.light` → `html[data-skin="X"]...`). The browser resolves them; jsdom does not. Estimating "looks fine" is not an audit.
- Rule: Read `index.css` with `fs`, parse selector→{prop:value}, flatten the token map for each context's selector chain (increasing specificity, last wins), and `resolveVars()` transitively — the exact approach in `.claude/verify-css-equiv.cjs`. Then COMPUTE the WCAG 2.1 relative-luminance ratio (`(L_hi+0.05)/(L_lo+0.05)`, sRGB linearization) of `--body-fg`/`--body-bg` (≥4.5:1, SC 1.4.3 text) and the accent `--neon-green`/`--body-bg` (≥3:1, SC 1.4.11 non-text / large text). Derive contexts from `SKIN_IDS` (not a hardcoded list) so a new skin is audited automatically. THREE non-negotiables: (1) assert the resolved value is opaque hex and throw a clear error otherwise — alpha (`rgba` panel tokens) would silently mis-luminance, so don't audit those pairs; (2) assert no leftover `var(` in the resolved value, else a typo'd token name passes vacuously; (3) prove the guard discriminates — black/white must compute 21.00 and a deliberately bad pair must compute <3:1 and trip the threshold (a green test that can't go red is theater). See `theme-contrast-audit.test.js`. Measured headroom on the shipped palettes: body text 11.35–14.20:1, accent 3.43–17.15:1 (tightest analog-light 3.43:1).

### [Claude-4] A .ps1 written by a non-PowerShell tool MUST be ASCII (or UTF-8-with-BOM) — em-dashes break Windows PowerShell 5.1 parsing
- What happened: Rewrote `next-swarm.ps1` via the Write tool (UTF-8, no BOM) with em-dashes (`—`, U+2014) in comments AND in one Write-Host string. `[Parser]::ParseFile` failed with "The string is missing the terminator: \"." pointing at line 118 — a line whose quotes were perfectly balanced. Total ASCII `"` count was even; no odd-quote lines. Replacing every non-ASCII byte with ASCII (`—`→`--`) made it parse instantly.
- Root cause: Windows PowerShell 5.1 (`powershell.exe`) reads a script with NO BOM as the system ANSI code page (CP1252), not UTF-8. A 3-byte UTF-8 em-dash (E2 80 94) is decoded as three CP1252 chars; when that sits inside a double-quoted string (line 105 here), the mojibake desynchronizes the tokenizer's quote tracking, and the "unterminated string" surfaces at the NEXT string literal downstream (line 118) — so the reported line is a red herring. Comments don't protect you: the corrupting byte was in a real string. Note `[^\x00-\x7F]` / grep `"` analysis looks clean because the tooling reads the file as UTF-8 while PowerShell does not.
- Rule: Any `.ps1` authored by Claude/Write (or any editor that omits the BOM) must be pure ASCII, OR be saved as UTF-8 with BOM. Don't paste `—`, `–`, “smart quotes”, `…`, or non-breaking spaces into PowerShell — use `--`, `"`, `...`. Validate every generated/edited .ps1 with `[System.Management.Automation.Language.Parser]::ParseFile(path,[ref]$t,[ref]$e)` and a `grep -nP '[^\x00-\x7F]'` sweep before declaring it done. When a "missing terminator" points at a balanced line, suspect an encoding-mangled multibyte char on an EARLIER line, not that line.
