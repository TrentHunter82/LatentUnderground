...
class TestSSEStreaming:
    ...
    async def test_stream_generator_delivers_lines(self):
        ...
        project_id = 42
        _project_output_buffers[project_id] = ["[stdout] Line 1", "[stdout] Line 2"]

        # Test the buffer read logic inline (simulates what the generator does)
        events = []
        offset = 0
        buf = _project_output_buffers.get(project_id, [])
    ...
    async def test_stream_done_when_no_agents(self):
        """Generator should emit done event when buffer is empty and no agents alive."""
        from app.routes.swarm import _project_output_buffers, _any_agent_alive

        project_id = 43
        _project_output_buffers[project_id] = []

        # With no agent processes, _any_agent_alive returns False → done event
        buf = _project_output_buffers.get(project_id, [])
        offset = 0
        if offset >= len(buf):
    ...
    async def test_stream_endpoint_exists(self, client, created_project):
        ...
        from app.routes.swarm import _project_output_buffers
        _project_output_buffers[pid] = ["test line"]
    ...
    async def test_polling_output_still_works(self, client, created_project):
        ...
        pid = created_project["id"]
        from app.routes.swarm import _project_output_buffers
        _project_output_buffers[pid] = ["line A", "line B", "line C"]
    ...
    async def test_stream_buffer_mutation(self):
        ...
        project_id = 44
        _project_output_buffers[project_id] = ["initial"]

        buf = _project_output_buffers[project_id]
        # Simulate first read
        offset = 0
        batch1 = buf[offset:]
        offset = len(buf)
