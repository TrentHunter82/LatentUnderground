...
- [x] Add tests for templates CRUD API (create, list, get, update, delete, not found)
- [x] Add tests for process reconnection (alive PID reattach, dead PID cleanup)
- [x] Add tests for swarm output pagination (offset, limit, empty buffer)
- [x] Fix flaky tests: Migrate all test fixtures from hardcoded paths to tmp_path
- [x] Add frontend tests for template selector, sparklines, virtualized log
- [x] Signal: Create .claude/signals/tests-passing.signal

## Claude-4 [Polish/Review] - Final quality gate

- [x] Review all Phase 8 code changes for quality and consistency
- [x] Verify no regressions: all Phase 7 tests still pass (503+)
- [x] Accessibility review: keyboard nav, screen reader support, ARIA on new components
- [x] Performance profiling: measure render times for large log files, API response times
- [x] FINAL: Generate next-swarm.ps1 for Phase 9 or mark project production-ready
...
- [x] Log viewer handles 1000+ lines without UI lag
- [x] All tests pass (backend + frontend), zero flaky tests
